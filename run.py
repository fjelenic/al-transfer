...
def set_warm_start(X_pool, y_pool, warm_start_size, X_pair_pool=None):
    X_labeled = []
    y_labeled = []
    X_pair_labeled = [] if X_pair_pool else None
    random_sampler = RandomSampler()
    warm_start_inds = random_sampler.select_batch(np.array(X_pool), warm_start_size, X_pair=np.array(X_pair_pool)).tolist()
...
def fill_labeled_pool(inds, mapping, X_pool, y_pool, X_labeled, y_labeled, X_pair_pool=None, X_pair_labeled=None):
    ...
    return dict(Counter(y_labeled[-len(inds):])), list(map(mapping.get, X_labeled[-len(inds):]))
...
def get_al_kwargs(X_pool, X_labeled, model, batch_size, criterion, X_pair=None, X_pair_lab=None):
    al_kwargs = dict(
            X_unlab=X_pool,
            X_lab=X_labeled,
            model=model,
    ...
    for data in datasets:
        ...
        for i, model_cls in enumerate(models):
            ...
            for seed in range(20):
                X_pool, X_test, X_pair_pool, X_pair_test, y_pool, y_test, mapping = load_data(data)
                res = []
                selected_labels = []
                selected_examples = []
                set_seed(seed)
                X_labeled, y_labeled, X_pair_labeled = set_warm_start(X_pool, y_pool, batch_size, X_pair_pool)
                selected_labels.append(dict(Counter(y_labeled)))
                selected_examples.append(list(map(mapping.get, X_labeled)))
                model = model_cls(data.num_out, device=torch.device(GPU)) if model_names[i] in ["BERT", "ELECTRA", "RoBERTa"] else model_cls()
                if model_names[i] in ["BERT", "ELECTRA", "RoBERTa"]:
                    ...
                    criterion = None
                model.train_loop(X_labeled, y_labeled, criterion=criterion, X_pair=X_pair_labeled, num_epochs=5)
                res.append(eval_model(model, X_test, y_test, data.num_out, X_pair_test=X_pair_test))
                for iteration in range(data.num_iter):
                    print(f"{data.name}-{al.name}-{model.name}-{seed}-{iteration}/{data.num_iter}")
                    if model_names[i] in ["BERT", "ELECTRA", "RoBERTa"]:
                        al_kwargs = get_al_kwargs(X_pool, X_labeled, model, batch_size, criterion, X_pair=X_pair_pool, X_pair_lab=X_pair_labeled)
                    else:
                        X_pool_vec = model.vectorize_data(X_pool, X_pair=X_pair_pool)
                        X_labeled_vec = model.vectorize_data(X_labeled, X_pair=X_pair_labeled)
                        al_kwargs = get_al_kwargs(X_pool_vec, X_labeled_vec, model, batch_size, criterion)
                    inds = al.select_batch(**al_kwargs).tolist()
                    labs_count, example_data = fill_labeled_pool(inds, mapping, X_pool, y_pool, X_labeled, y_labeled, X_pair_pool=X_pair_pool, X_pair_labeled=X_pair_labeled)
                ...
                results[data.name]["random"][model.name]["labels"].append(selected_labels)
                results[data.name]["random"][model.name]["examples"].append(selected_examples)
        ...
        for al in al_methods[1:]:
            for i, model_a_cls in enumerate(models):
                if model_names[i] == "BERT" or model_names[i] == "ELECTRA":
                    continue
                res = {name:{"accuracy":[[] for _ in range(20)]} for name in model_names}
                ...
                for seed in range(20):
                    ...
                    model_a = model_a_cls(data.num_out, device=torch.device(GPU)) if model_names[i] in ["BERT", "ELECTRA", "RoBERTa"] else model_a_cls()
                    if model_names[i] in ["BERT", "ELECTRA", "RoBERTa"]:
                        criterion = nn.BCEWithLogitsLoss() if data.num_out == 1 else nn.CrossEntropyLoss()
                        criterion.to(model_a.device)
                    else:
                        criterion = None
                    model_a.train_loop(X_labeled, y_labeled, criterion=criterion, X_pair=X_pair_labeled, num_epochs=5)
                    res[model_names[i]]["accuracy"][seed].append(eval_model(model_a, X_test, y_test, data.num_out, X_pair_test=X_pair_test))
                    for j, model_s_cls in enumerate(models):
                        if i == j:
                            continue
                        else:
                            model_s = model_s_cls(data.num_out, device=torch.device(GPU)) if model_names[j] in ["BERT", "ELECTRA", "RoBERTa"] else model_s_cls()
                            if model_names[j] in ["BERT", "ELECTRA", "RoBERTa"]:
                                criterion = nn.BCEWithLogitsLoss() if data.num_out == 1 else nn.CrossEntropyLoss()
                                criterion.to(model_s.device)
                            else:
                                criterion = None
                            model_s.train_loop(X_labeled, y_labeled, criterion=criterion, X_pair=X_pair_labeled, num_epochs=5)
                            res[model_names[j]]["accuracy"][seed].append(eval_model(model_s, X_test, y_test, data.num_out, X_pair_test=X_pair_test))
                    for iteration in range(data.num_iter):
                        print(f"{data.name}-{al.name}-{model_a.name}-{seed}-{iteration}/{data.num_iter}")
                        if model_names[i] in ["BERT", "ELECTRA", "RoBERTa"]:
                            al_kwargs = get_al_kwargs(X_pool, X_labeled, model_a, batch_size, criterion, X_pair=X_pair_pool, X_pair_lab=X_pair_labeled)
                        else:
                            X_pool_vec = model_a.vectorize_data(X_pool, X_pair=X_pair_pool)
                            X_labeled_vec = model_a.vectorize_data(X_labeled, X_pair=X_pair_labeled)
                            al_kwargs = get_al_kwargs(X_pool_vec, X_labeled_vec, model_a, batch_size, criterion)
                        inds = al.select_batch(**al_kwargs).tolist()
                        labs_count, example_data = fill_labeled_pool(inds, mapping, X_pool, y_pool, X_labeled, y_labeled, X_pair_pool=X_pair_pool, X_pair_labeled=X_pair_labeled)
                ...
                results[data.name][al.name][model_a.name]["examples"] = selected_examples
                with open('results.pkl', 'wb') as f:
                    pickle.dump(results, f)
